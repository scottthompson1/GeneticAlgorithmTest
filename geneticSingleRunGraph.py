...
def main():
    ...
    plt.plot(range(len(results[0])), results[0], 'r-', range(len(results[1])), results[1], 'b-', range(len(results[2])), results[2], 'g-', range(len(results[3])), results[3], 'm-', range(len(results[4])), results[4], 'c-',)
    plt.plot(range(len(results[5])), results[5], color = '#420690', linestyle = '-', linewidth = 2)
